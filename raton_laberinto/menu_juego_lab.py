...
# Añadimos las carpetas de los juegos al path para poder importarlos
sys.path.append(os.path.join(os.path.dirname(__file__), 'profundidad'))
sys.path.append(os.path.join(os.path.dirname(__file__), 'anchura'))

try:
    from juego_laberinto_gui import iniciar_juego_anchura # Desde anchura/juego_laberinto_gui.py
except ImportError:
    def iniciar_juego_anchura():
        tk.messagebox.showerror('Error', 'No se encontró la versión Anchura')

try:
    from juego_laberinto_pr_gui import iniciar_juego_profundidad # Desde profundidad/juego_laberinto_pr_gui.py
except ImportError:
    def iniciar_juego_profundidad():
        tk.messagebox.showerror('Error', 'No se encontró la versión Profundidad')
...
def crear_menu_principal(master_window):
    ...
    title = tk.Label(header, text='Raton y el Queso', font=FONT_TITLE, bg=BG, fg=NEON_PRIMARY)
    title.pack()
    ...
    info_label = tk.Label(info_frame, text='Selecciona una opción para poner el queso. Atajos: M = Profundidad, B = BFS, Esc = Salir',
    ...
    def make_button(parent, title_text, desc, cmd, key_hint=None):
        ...
        def on_leave(e):
            info_label.config(text='Selecciona una opción para probar. Atajos: M = Profundidad, B = BFS, Esc = Salir')
    ...
    btn_minimax = make_button(main_frame, 'Laberinto (Profundidad)',
                              'IA invencible que usa el algoritmo Profundidad. Ideal para un verdadero desafío.',
                              lambda: lanzar_juego(iniciar_juego_profundidad, root), key_hint='M')

    btn_bfs = make_button(main_frame, 'Laberinto (Busqueda por anchura)',
                          'IA con estrategia simple basada en búsqueda en anchura. Buena para entender la IA.',
                          lambda: lanzar_juego(iniciar_juego_anchura, root), key_hint='B')
